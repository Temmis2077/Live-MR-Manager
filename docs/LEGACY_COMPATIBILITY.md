...
OSW는 이전 제품의 사용자 데이터를 가져오거나 옮기지 않습니다. 이미 배포된 외부
연결과 개발 도구 중 즉시 끊을 수 없는 식별자만 제한적으로 입력 호환합니다. 새
UI·문서·릴리즈 링크·생성되는 콜백에는 사용하지 않습니다.
...
새 코드에서 레거시 값을 추가할 때는 신규 출력이 아니라 외부 입력 호환·도구
폴백·법적 고지 중 하나인지 명시해야 합니다. 사용자에게 생성하거나 안내하는 저장소 주소,
